...
describe 'Authentication', type: :request do
  describe 'POST /authenticate' do
    ...
    it 'authenticates the client' do
      ...
      expect(response_body).to eq({ 'token' => response_body['token'] })
    end
    ...
    it 'returns error when username is missing' do
      ...
      expect(response.body).to eq("{\"error\":\"param is missing or the value is empty: username\"}")
    end

    it 'returns error when passoword is missing' do
      post '/api/v1/authenticate', params: { username: 'BookSeller99' }

      expect(response).to have_http_status(:unprocessable_entity)
      expect(response.body).to eq("{\"error\":\"param is missing or the value is empty: password\"}")
    end
